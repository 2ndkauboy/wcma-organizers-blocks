...
if ( empty( $organizers ) ) {
	...
	printf(
		...
		esc_html__( 'No organizers found. Make sure there are users with published posts.', 'wcma-organizers-block-js' )
	);
...
foreach ( $organizers as $organizer ) {
	...
	$item_class = 'wp-block-wcma-organizers-list-js__item';
	if ( $show_avatar ) {
	...
	if ( $show_avatar ) {
		$html .= '<div class="wp-block-wcma-organizers-list-js__avatar">';
		$html .= get_avatar( $organizer->ID, $avatar_size );
		$html .= '</div>';
	}

	$html .= '<div class="wp-block-wcma-organizers-list-js__content">';

	if ( $show_name ) {
		$html .= '<h3 class="wp-block-wcma-organizers-list-js__name">';
		$html .= esc_html( $name );
		$html .= '</h3>';
	}
	...
	if ( $show_bio ) {
		...
		if ( $bio ) {
			$html .= '<p class="wp-block-wcma-organizers-list-js__bio">';
			$html .= esc_html( $bio );
			$html .= '</p>';
		}
	}
	...
	if ( $show_link ) {
		$html .= '<a class="wp-block-wcma-organizers-list-js__link" href="' . esc_url( get_author_posts_url( $organizer->ID ) ) . '">';
		if ( '' !== $link_text ) {
			$html .= esc_html( $link_text ) . ' ' . esc_html( $organizer->first_name );
		} else {
			$html .= sprintf(
			/* translators: %s: author display name */
				esc_html__( 'Posts by %s', 'wcma-organizers-block-js' ),
